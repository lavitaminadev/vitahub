...
export class ReportingController {
  ...
  private resolveClientScope(req: AuthenticatedRequest): string | null {
    return req.user?.role === UserRole.CLIENT ? req.user.clientId ?? null : null;
  }
  ...
  async dashboard(@Req() req: AuthenticatedRequest) {
    ...
    const [pieceRows] = await this.dataSource.query(
      `SELECT status, COUNT(*) as count FROM pieces WHERE organization_id = ? GROUP BY status`,
      [orgId],
    );
    const [xpRow] = await this.dataSource.query(
      `SELECT COALESCE(SUM(amount),0) as total FROM xp_events WHERE user_id IN (SELECT id FROM users WHERE organization_id = ?) AND created_at >= DATE_SUB(NOW(), INTERVAL 1 MONTH)`,
      [orgId],
    );
    const [udRow] = await this.dataSource.query(
      `SELECT COALESCE(SUM(contracted),0) as contracted, COALESCE(SUM(consumed),0) as consumed, COALESCE(SUM(reserved),0) as reserved FROM ud_budgets WHERE client_id IN (SELECT id FROM clients WHERE organization_id = ?)`,
      [orgId],
    );

    const pendingPieces = (Array.isArray(pieceRows) ? pieceRows : []).reduce(
      (sum: number, p: any) => (p.status !== 'delivered' && p.status !== 'cancelled' ? sum + Number(p.count) : sum),
      0,
    );
    const pieces = (Array.isArray(pieceRows) ? pieceRows : []).map((p: any) => ({
      status: p.status,
  ...
  async reports(@Req() req: AuthenticatedRequest) {
    ...
    const [monthlyRows] = await this.dataSource.query(
      `SELECT DATE_FORMAT(created_at, '%Y-%m') as month, COALESCE(SUM(ud_amount),0) as ud
       FROM pieces WHERE ${pieceConditions.join(' AND ')} AND created_at >= DATE_SUB(NOW(), INTERVAL 12 MONTH)
    ...
    const [topRows] = await this.dataSource.query(
      `SELECT c.name, COALESCE(SUM(p.ud_amount),0) as revenue
       ...
       WHERE ${clientId ? 'c.organization_id = ? AND c.id = ?' : 'c.organization_id = ?'}
  ...
  async kpi(@Req() req: AuthenticatedRequest) {
    ...
    const [utilRow] = await this.dataSource.query(
      `SELECT COALESCE(AVG(current_load * 100.0 / NULLIF(capacity, 0)), 0) as pct FROM pods WHERE organization_id = ?`,
      [orgId],
    );
